...
- Orchestrator curated samples such as `agent_scheduler_delay_sample.json` and `remote_fallback_recovery_sample.json` are downstream reviewer navigation artifacts. They can map to AIGuard `scheduler_delay_pattern` and `remote_execution_recovered_by_fallback`, but they are not Runtime benchmark outputs, `result.json` schema inputs, or deployment policy signals.
